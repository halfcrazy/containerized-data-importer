...
func (r *ImportReconciler) updateStatusPhase(pvc *corev1.PersistentVolumeClaim, dataVolumeCopy *cdiv1.DataVolume, event *Event) error {
	...
	case string(corev1.PodFailed):
		// Only mark as Failed for fatal errors (e.g., checksum mismatch).
		// Other failures keep the current phase to allow retries.
		if pvc.Annotations[cc.AnnRunningConditionReason] == "ChecksumError" {
			dataVolumeCopy.Status.Phase = cdiv1.Failed
		}
		// Otherwise, keep the current phase (e.g., ImportScheduled) to allow retries
